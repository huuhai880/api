...
class tin
{
    ...
    public static function CapNhatKetQuaCacTin(string $ten_tai_khoan, string $loai_tai_khoan )
    {
        ...
        $missing_tinIDs = array_diff($tinIDs, array_keys($ds_chi_tiet_bulk));

        $missing_tinIDs_str = implode(',', $missing_tinIDs);

        if (!empty($missing_tinIDs)) {
            
            $data_chi_tiet_tin = chi_tiet_tin::lay_chi_tiet_cua_tin($missing_tinIDs_str);

            $groupedData = array();
            // Loop through the data array and count occurrences of each id_tin
            foreach ($data_chi_tiet_tin as $item) {
                $id_tin = $item->id_tin;
            
                // If the id_tin is not already a key in the groupedData array, add it with an empty array
                if (!isset($groupedData[$id_tin])) {
                    $groupedData[$id_tin] = array();
                }
            
                // Add the current item to the array corresponding to its id_tin
                $groupedData[$id_tin][] = $item;
            }
            

            $ds_chi_tiet_bulk = $groupedData;
        ...
        foreach ($tins as $tin) {
            ...
            $ds_chi_tiet = $ds_chi_tiet_bulk[$tin->id] ?? [];

            $result = tin::CapNhatKetQuaTin($tin, $ds_chi_tiet, "SAVE");
        }
    ...
    public static function LayTinChuaCoKetQua(string $ten_tai_khoan, string $loai_tai_khoan): array
    {
        ...
        $sql = "SELECT * FROM tin WHERE trang_thai = -1  AND vung_mien = 'mb'";
    ...
    public static function CapNhatThongKeChoTin(tin $tin, array $ds_chi_tiet): array
    {
        ...
        foreach ($ds_chi_tiet as $chi_tiet_tin) {
            ...
            if ($chi_tiet_tin->kieu === "blo") {
                ...
                $chi_tiet_tin->xac += $so_lo * $chi_tiet_tin->diem * $so_luong_so * 100; //Xác = số_lô * điểm * số lượng số. số lô miền nam là 18,17,16, mb 27 23 20 
    ...
    {   
        ...
        $sql_lay_tin = "SELECT * FROM tin WHERE tai_khoan_danh = '$ten_tai_khoan'
                        AND vung_mien='mb'  AND trang_thai != -1 ";     
    ...
    public static function CapNhatKetQuaTin(tin $tin, array $ds_chi_tiet, $type): array
    {
        $result = array();
        $da_co_ket_qua = true;
